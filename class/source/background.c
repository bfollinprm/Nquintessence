 ...
 */
 ...
 /**ADDITION BY BRENT FOLLIN
 * Subroutine calculating the hubble rate including effects of the chi field.
 * 
 *
 * This function passes C arguments to some easily modified python code, background.py.
 *
 *
 *
 *
 *
 * @param h                        Input : reduced hubble rate (km/s/Mpc)
 * @param omega_m                  Input : physical total matter density today: \Omega_m h^2
...
//printf("Args are h = %f, omega_m = %f, omega_rad = %f, chi0 = %f, m_chi_over_H0 = %f\n", h, omega_m, omega_rad, chi0, m_chi_over_H0);
  ...
  check = PyTuple_SetItem(py_tuple_Function_Args, 1, PyFloat_FromDouble(omega_m));
  if (check)
  {
    return 1;
  }
  check = PyTuple_SetItem(py_tuple_Function_Args, 2, PyFloat_FromDouble(omega_rad));
  if (check)
  {
    return 1;
  }
  check = PyTuple_SetItem(py_tuple_Function_Args, 3, PyFloat_FromDouble(chi0));
  if (check)
  {
    return 1;
  }
  check = PyTuple_SetItem(py_tuple_Function_Args, 4, PyFloat_FromDouble(m_chi_over_H0));
  if (check)
  {
    return 1;
  }
...
int background_functions(
  ...
  double redshift;
  redshift = 1.0/a_rel -1.0;
  PyObject *tuple_args = PyTuple_New(1);
  PyTuple_SetItem(tuple_args, 0, PyFloat_FromDouble(redshift));
  // if (!PyFloat_Check(PyTuple_GetItem(tuple_args, 0)))
  // {
